...
const SESSION_PATH = resolve(process.env.TXLINE_SESSION_PATH || "work/txline-mainnet-session.json");
...
async function ensureBalance(keypair) {
  ...
  if (balance >= 0.004 * LAMPORTS_PER_SOL) return balance;
  fail(`Insufficient SOL on Mainnet. Please send at least 0.005 SOL to ${keypair.publicKey.toBase58()}`);
  return balance;
}
...
async function syncHistorical(session, fixtureId, endpointArgument, metadataOnly = false) {
  ...
  const importOne = async () => {
    ...
    try {
      ...
      if (!response.ok || !text.trim()) fail(`${response.status}: ${text.slice(0, 2_000)}`);
